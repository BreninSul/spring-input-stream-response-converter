...
@TestConfiguration
class ControllerTestDefault {

    @Test
    fun testLogFullInfoRequest(){
        val  request = HttpRequest.newBuilder()
            .uri(URI.create("http://127.0.0.1:5367/test-controller-default"))
        ...
        assert(realLines[2].startsWith("=URI          : POST /test-controller-default"))
    ...
    fun testLogFullInfoResponse(){
        ...
        val realLines=lines.subList(firstLine,lines.size)
        assert(realLines[1].startsWith("=ID"))
        assert(realLines[2].startsWith("=URI          : 200 POST /test-controller-default"))
